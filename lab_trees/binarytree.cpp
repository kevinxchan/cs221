...
template <typename T>
void BinaryTree<T>::printLeftToRight(Node* root) const {
    if (root != NULL) {
...
{
    return sumDistances(root, 1, height());;
}

template <typename T>
int BinaryTree<T>::sumDistances(Node* root, int acc, int h) const {
    if (root == NULL)
        return 0;
    int counter = 0;
    while (counter < h) {
        acc = acc + sumDistances(root->left, acc, h);
        // acc = acc + sumDistances(tmp_root->right, acc, h--);
        counter++;
    }
    return acc;
}
